...
measurement that corrected this project's earlier explanation of why.
...
`game_enter` (`src/keys.ts` + the wiring in `src/index.ts`) is the answer to a gap the base console
tools leave open: the console endpoint answers from the title screen, but nothing could get *past*
it. A probe run against the real game on 2026-08-18 measured two things that shape everything below:
...
**The keystroke-leak defect the previous design accepted is gone.** `SendInput` targets whatever
window has OS foreground focus, so the earlier design knowingly accepted that a press could land in
whatever the user had alt-tabbed to. The script now brings the game window to the front (minimize →
restore → `SetForegroundWindow`, because a bare `SetForegroundWindow` is refused but restoring a
minimized window is permitted) and injects **only** once `GetForegroundWindow()` has been read back
and found to be the game's own window. If the foreground cannot be taken it injects nothing at all
and posts to the HWND instead, which can't leak either. What it costs instead is that the game window
is pulled to the front — unavoidable for the routes above, and it needs no human, which is the
...
deliberate, narrow exception: it is the only route measured to move this screen. Not because the
title screen precedes every hook the DLL installs — see below.

### The hook is there; the title screen just does not read it

Measured 2026-08-18, at the end of a fix wave, because nobody had ever measured the premise the
`SendInput` exception rests on. The polled guards attach with the graphics hooks, and the game
...
Why is not established, and this file is not going to invent a third confident guess about this
...
`successfully changed world to: orbit_d2` line. Without a `character` argument, **it leaves the game
at the character-selection screen**. On failure it reports which stage it stopped at (`launch`,
`titleScreen`, `keyPress`, `worldLoad`, or `ambiguous` — see below), since that's what a calling
...
title screen. This was an Important finding from a review round, confirmed real and not
...
`game_enter` called it fire-and-forget, without checking success. The prior in-process variable this
replaced could never fail to record a press within a process's own lifetime — a plain assignment
...
priority checks: a world already loaded always wins over a stale press record, and — fixed in a
later review round, since a `tasklist` pid hiccup on a game that's already fully in orbit has nothing
...
before running it, or see the environment note in `README.md`.
...
`scripts/struct-read-live.mjs` is the same idea for the capability layer: it reads `mem.module` for
the running image base, then runs the real `struct_read` capability against it and asserts the PE
header's `e_magic` decodes to `0x5A4D` ("MZ"). It is a throwaway driver, not part of
`test:capabilities` — it needs the game and Windows node, same as `scripts/smoke.mjs`, and opens the
one endpoint connection, so nothing else may hold it while it runs.
...
Both `endpoint.ts` (directly, via `scripts/smoke.mjs`) and the full stdio round trip through the MCP
SDK have now been run against the real, running game. In summary: a 16-check live round trip
(registry with 18 entries and numeric bounds, read, write, read-back, `outOfRange` with the value
provably unchanged, `unknownName`, `badArgument`, the client-side over-long guard refusing before
anything reached the socket, two concurrent requests resolving to their own rows) passed in 0.22s;
and the agent-shaped sequence — `game_launch` (window up, pid reported) → `console_describe` →
`console_run` write → `console_run` read-back → `log_read` (real log content) → `game_kill` —
exercising five of the six base tools, passed end to end in 20.9s, including `launchGame()`'s
PowerShell JSON parsing working on the first try. Not yet exercised at the time: the
retry-on-connect-failure path (see "Retry policy" above).

`game_enter`'s full sequence is proven against the real game (2026-08-18). Driven over stdio from a
closed game it answered
`{"status":"ok","route":"sendInput","message":"The game reached the character-selection screen."}`
...
foreground is not optional for this engine — see "What the 2026-08-18 re-measurement changed" above.

The capability layer's first tool, `struct_read`, is likewise verified live (`scripts/struct-read-live.mjs`):
given the running image base from `mem.module`, it decoded the PE header's `e_magic` field as
`0x5A4D` ("MZ") through one real `mem.read` round trip via the `ctx.mem` façade.

The endpoint's one-connection-at-a-time refusal is also measured directly, 2026-08-19: in orbit,
over that one connection, `console_describe`, `character.list`, `mem.module` and an 18 MB
`mem.scan_heap` all answered normally. The "endpoint goes into a permanent busy loop in orbit"
story recorded for about a week beforehand was never an orbit fault — it was `game_enter` opening
the MCP's own connection at sign-in, which is the moment any *other* client stops being served. See
`README.md`'s tools table and the wire-protocol section above for the `holder_port` mechanism that
makes this diagnosable now instead of looking like a timeout.
...
- `src/capabilities/contract.ts` — the `Capability`, `CapabilityContext`, `SunriseConsole`,
  `MemFacade` and `GameFacade` types every capability is written against.
- `src/capabilities/mem.ts` — `createMemFacade`: chunks `mem.read` at 256 bytes
  (`probe::kMaxReadBytes`), reassembles the hexdump rows into a contiguous `Uint8Array`, sorting by
  row address so out-of-order rows still concatenate right.
- `src/capabilities/context.ts` — `buildContext`, which assembles the real `CapabilityContext` from
  a live `SunriseEndpointClient`: wires `console`/`mem`/`game`/`log` to the same primitives the base
  tools use.
- `src/capabilities/register.ts` — `registerCapabilities`, the loader that registers each
  `Capability` as an MCP tool via `server.registerTool`.
- `src/capabilities/index.ts` — the barrel: the `CAPABILITIES` array every capability is added to.
  One import, one entry, per capability.
- `src/capabilities/struct-read.ts` — the `struct_read` capability: parses a decimal-or-hex address
  and a field list, reads the covering span through `ctx.mem.read`, and decodes each field
...
- `scripts/character-smoke.mjs` — the test for `character.ts` plus the `game_kill` settle: the
  ...
  `npm run test:keys`.
- `scripts/smoke.mjs` — the live-game counterpart for `endpoint.ts`, described in "Testing against a
  live game".
- `scripts/mem-facade-smoke.mjs`, `scripts/struct-read-smoke.mjs`, `scripts/context-smoke.mjs`,
  `scripts/register-smoke.mjs` — the fake-`ctx` smoke tests for the capability layer, wired into
  `npm run test:capabilities`.
- `scripts/struct-read-live.mjs` — the live-game counterpart for the capability layer, described in
  "Testing against a live game" above.

---

## Appendix: pre-existing fix-round history

The rest of this file predates the README/NOTES/AGENTS restructuring above and is kept verbatim as
a record of real review rounds against the endpoint client — do not delete evidence just because
it's old.
...
design/architecture content (retry policy, testing approach, known limits) lives in the sections
above instead — this part of the file is the "what changed and why" history of that round.
...
pending request — see "Retry policy" above for the full reasoning). Wiring this through
...
see "On testing dispatch correctness" above); a new test asserts a 600-byte line is rejected
...
**Docs** — `NOTES-task7.md` (task-scoped) removed; its durable content folded into the sections
above ("Retry policy" and "Design notes and known limits", plus an expanded testing section). This
file replaces it as the record of a specific review round, deliberately without task numbers, so it
stays legible after the originating plan document is gone.
...
and what changed as a result. The "Verified against the live game" section above has the durable
summary of the results themselves.
...
- The retry logic added in the first review round (Finding 1) was never actually exercised:
  ...
  launch path later — but claiming it as load-bearing today would be dishonest. "Retry policy" above
  says so plainly.
...
- **Owned and strengthened `scripts/smoke.mjs`.** Two of its checks were weaker than they looked:
  ...
  error — worth stating plainly rather than assuming it gets read first). Committed as the Step 2
  deliverable the plan asks for.
...
- **Replaced the two `task-7-brief.md` references in `src/endpoint.ts`** (lines 9 and 21) with
  either the substance or a pointer into this documentation — the same defect as the commit subject
  fixed in the first round, missed in that sweep. `task-7-brief.md` lives in `sunrise-console`'s plan
  directory and won't survive the plan being finished.
- **Made `close()` actually fast, instead of documenting around the fact that it wasn't.** The
  ...
  the docs at the time was aspirational, not yet true. Fixed the code instead of softening the docs:
  `close()` now aborts an `AbortController` that both the reconnect-delay sleep
  (`node:timers/promises`' `setTimeout` with `{ signal }`) and the TCP connect attempt
...
- **Corrected an earlier claim about what the close-during-delay test proves** — see the "second
